...
const GroupingSection: React.FC<GroupingSectionProps> = ({ participants }) => {
  ...
  const handleDownloadCSV = async () => {
    ...
    const fileName = `分組結果_${new Date().toISOString().slice(0, 10)}.csv`;
    const bomCvsContent = "\uFEFF" + csvContent;

    try {
      // 嘗試使用現代瀏覽器的 "另存新檔" API
      // @ts-ignore - Window interface might not have showSaveFilePicker types defined depending on environment
      if (window.showSaveFilePicker) {
        ...
        await writable.write(bomCvsContent);
        await writable.close();
        return;
      }
    } catch (err) {
      // 如果用戶取消或不支援，則忽略錯誤，繼續執行下面的 Fallback
      if ((err as Error).name === 'AbortError') return;
      console.warn('File System Access API failed, using fallback:', err);
    }

    // Fallback: 傳統下載方式 (通常會存到預設下載資料夾)
    const blob = new Blob([bomCvsContent], { type: 'text/csv;charset=utf-8;' });
